<?php
    ...
    */
    ...
    $conexao = new Conecxao();
    $conexao->Conectar();

    $insert = new Inserir();//Permissão para acessar os métodos da classe inserir.
    echo $insert-> cadastrar($conexao, "pessoa","Laurindozinhozin","11979669066");
    
    $select = new Consultar();//Permissão para acessar os métodos da classe consultar.
    echo $select->consultarIndividual($conexao,"pessoa",1);
    echo $select->consultarTudo($conexao,"pessoa");

    $atu = new Atualizar();//Permissão para acessar todos os métodos da classe Atualizar.
